    ...
    let $dataset := 
        if($pid) then 
                let $firstDegree := collection($config:data-root)//tei:relation[
                        ...
                        @mutual[matches(.,$id)]]                                   
                let $firstIDs := string-join(($firstDegree/@active, $firstDegree/@passive,$firstDegree/@mutual),' ')
                let $firstIDs := distinct-values((tokenize($firstIDs,' ')))
                let $secondDegree := 
                    if($degree = 'second') then
                        for $i in $firstIDs
                        let $c := subsequence(collection($config:data-root)//tei:relation[
                                    @passive[matches(.,concat($i,'(\W.*)?$'))] or 
                                    @active[matches(.,concat($i,'(\W.*)?$'))] or
                                    @mutual[matches(.,concat($i,'(\W.*)?$'))]],1,10)
                        return $c
                    else ()    
           return 
                if($degree = 'second') then
                    functx:distinct-nodes(($secondDegree))
                else functx:distinct-nodes($firstDegree)
